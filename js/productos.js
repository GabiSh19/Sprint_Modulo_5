...
let categoriaProductos = JSON.parse(
  localStorage.getItem('categoriaProductos')
) || [{}]
...
addEventListener('DOMContentLoaded', async () => {
      ...
      items.forEach(producto => {
        ...
        div.innerHTML = `
                      ...
                      <div class="card-body">
                          ...
                          <p class="categoria_prod">Categoria:${mostrarCategoria(
                            idCategoria
                          )}</p>
  ...
  async function ListadoCategoria () {
    ...
    data = await respuesta.json()
    //AGREGADO
    categoriaProductos = data.map(data => {
    ...
    localStorage.setItem(
      'categoriaProductos',
      JSON.stringify(categoriaProductos)
    )
  }
  ...
  function mostrarCategoria (idCategoria) {
    let nombreProducto = categoriaProductos.find(item => item.id == idCategoria)
...
async function fetchData () {
  const response = await fetch(
    `https://slifer.bsite.net/td-producto/idSucursal/9`
  )
